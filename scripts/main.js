...
*/
const recipes = [
  { id:1, name:'Jollof Rice', category:'Breakfast', ingredients:['Rice','Tomato','Onion','Spices'], instructions:'Cook rice with tomato sauce and spices.', image:'images/dish1.jpg' },
  { id:2, name:'Egusi Soup', category:'Dinner', ingredients:['Melon Seeds','Spinach','Meat'], instructions:'Cook melon seeds with spinach and meat.', image:'images/dish2.jpg' },
  { id:3, name:'Pounded Yam', category:'Lunch', ingredients:['Yam','Water','Salt'], instructions:'Boil yam, then pound to desired texture.', image:'images/dish3.jpg' }
];
...
function filterRecipes(category){ displayRecipes(category); }
...
function addFavorite(id){
  ...
  if(list.includes(item.id)){ alert(`${item.name} is already in favorites.`); return; }
  list.push(item.id);
...
function initReservation(){
  ...
  form.addEventListener('submit',(e)=>{
    ...
    if(!name||!email||!date||!guests){ alert('Please complete all fields.'); return; }
...
function initContact(){
  ...
  form.addEventListener('submit',(e)=>{
    ...
    if(!name||!email||!msg){ if(resp) resp.textContent='Please complete all fields.'; return; }
    if(resp) resp.textContent=`Thank you ${name}! Your message has been sent.`; else alert(`Thank you ${name}! Your message has been sent.`);
    form.reset();
  });
}
